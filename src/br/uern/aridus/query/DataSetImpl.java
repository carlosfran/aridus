...
import br.uern.aridus.model.DataSetInterface;
...
public class DataSetImpl implements DataSetInterface {
	...
	private final static String VOID_ENDPOINT =
			"http://181.41.210.125:8080/openrdf-sesame/repositories/datasetkb";
	...
	public boolean create(DataSet dataset) {
		...
		String queryAsk = "ASK WHERE{ <" + dataset.getUri() + "> ?p ?o }";
		...
		try {
			...
			if (!bask) {
				String queryInsert = "PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>\n"
						...
						+ "\" ;\n"
						+ "\tvoid:sparqlEndpoint <"
						+ dataset.getSparqlEndpoint() + "> ;\n"
						+ "\tvoid:triples \""+dataset.getTriples()+"\"^^xsd:integer";
				Iterator<String> it = (dataset.getVocabulary()).iterator();
				while (it.hasNext()) {
					String v = it.next();
